...
DROP POLICY IF EXISTS "允许所有人查看评估数据" ON assessments;
DROP POLICY IF EXISTS "允许所有人插入评估数据" ON assessments;
DROP POLICY IF EXISTS "允许所有人更新评估数据" ON assessments;
DROP POLICY IF EXISTS "允许所有人删除评估数据" ON assessments;
...
SELECT '✅ 数据库清理并重建成功！' AS status,
       '✓ patients 表已创建' AS table1,
       '✓ assessments 表已创建' AS table2,
       '✓ 索引已创建' AS indexes,
